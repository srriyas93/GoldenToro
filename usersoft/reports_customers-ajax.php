...
if (isset($_GET['mode']))
{
    ...
    else  if ($_REQUEST['mode'] == "TransactionReport")
    {
      ...
      $cust_date_end= date("Y-m-d 23:59:59", strtotime($cust_end_date));
        ...
        $q = "SELECT cp.cust_id AS cid, cu.name AS cname, plans.title AS plname,cp.started_date AS entry,s.amount AS amt, s.settled_date AS tr_date from settlement s, customer_plans cp, users_customers cu, plans where cp.sno = s.cust_plan_id AND cu.cust_id = cp.cust_id AND plans.id = cp.plan_id AND cu.email='$username'";
