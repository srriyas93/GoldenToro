function WeeklySettlement(event)
{
	...
	var url = "weekly_settlement-ajax.php?mode=DisplayCustAmt&planid="+planid+"&dt="+dt; // the script where you handle the form input.
...
{
	var note =document.getElementById(sno+'settle_assign_note').value;
	...
	swal({
        ...
        .then((willDelete) => {
            if (willDelete)
            {
				var url = "weekly_settlement-ajax.php?mode=SettleAmt&cu_pl_id="+sno+"&amt="+amt+"&dt="+dt+"&status="+status+"&note="+note; // the script where you handle the form input.
				$.ajax({
					type: "POST",
					url: url,
					async: false,
